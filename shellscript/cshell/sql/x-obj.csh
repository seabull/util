...
#------------------------------------------------------------------------------
  ...
  set col_args   = ( t.Owner t.Object_Name t.SubObject_Name t.Object_Id )
  set col_args   = ( $col_args t.Data_Object_Id t.Object_Type t.TimeStamp )
  set col_args   = ( $col_args t.Status t.Temporary t.Generated )
  set col_args   = ( $col_args t.Created t.Last_DDL_Time )

  set cols       = ( owner name type )

#------------------------------------------------------------------------------

# Extended args that require subqueries

  set col_names  = ( $col_names alloc inuse free )
  set col_args   = ( $col_args xAllocated xIn_Use xFree )
...
#------------------------------------------------------------------------------
  ...
  if ( $?ts == 0 ) then
    ...
    set tsx = " $tsx exists
    set tsx = " $tsx ( select   'x'
    set tsx = " $tsx   from   dba_segments s "
    set tsx = " $tsx   where  s.segment_name = t.Object_Name "
    set tsx = " $tsx   and    s.segment_type = t.Object_Type "
    set tsx = " $tsx   and    ( "
    set cnt = 0
    foreach p ( $ts )
      @ cnt = $cnt + 1
      if ( $cnt > 1 ) set tsx = "$tsx Or "
      set tsx = " $tsx s.tablespace_name like Upper('$p') "
    end
    set tsx = " $tsx ) "
    foreach p ( $xts )
      set tsx = " $tsx and    s.tablespace_name not like Upper('$p') "
    end
    set tsx = " $tsx ) "
  endif
...
#------------------------------------------------------------------------------
  ...
  if ( $?show_sql == 0 ) then
    sqlplus -s $connect @$sql | tr -d '\014' | tee $out | list
    if ( $?print ) then
      print $out
    endif
  else
