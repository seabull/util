-- $Id: integrity.spb.sql,v 1.3 2008/01/14 15:42:55 yangl Exp $
...
--------------------------------------------------------------------------
   ...
   BEGIN
      ...
      THEN
         ...
         distQuery (by_name1, usrprinc, project, subproject, ndist, nsrc, uo, npct, dtype);
	...
	BEGIN
        ...
        THEN
            ...
            THEN
                ...
                -- qpct := wpct;
                traceit.log(constDEBUG_LEVELA, 'qpct=%s',qpct);
                --qpct := npct;
                qpct := wpct;
                payroll_pct := npct;
            ELSE
             ...
             *  residual(X) without changing pct.
             */
                ...
                -- ndist is not null
                -- got distribution from fall-thru hardcoded (project/subproject)
                -- instead of effort for effort-users.
                if (dtype = 2 and charge_by is null and wdist is not null) then
                    -- Following Effort and not creating new user account
                    ndist := wdist;
                    npct := wpct;
                end if;
            END IF;
        ELSE    -- wtype=0
            -- if not user account, set dist/src/pct to null
            ndist := NULL;
            nsrc := NULL;
            qpct := NULL;
        END IF;
