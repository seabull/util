-- $Header: c:\\Repository/database/rams/reports/adhoc/DetailY2D/machines.sql,v 1.3 2006/05/17 14:50:27 yangl Exp $
--
set termout off
set heading off
set linesize 4000
...
select
	...
	,case when row_number() over (partition by hr.hostname, hr.assetno, dist_vec, m.charge_by order by j.post_date desc, hr.usrprinc)=1 then
		...
		monthagg(j.post_date) over (partition by hr.hostname, hr.assetno, dist_vec, m.charge_by 
