-- $Id: alldetail_new.sql,v 1.2 2007/11/15 19:47:31 yangl Exp $
--
...
----
        ...
        ,hr.subproject
        ,c.post_date
    	,case when row_number() over (partition by hr.hostname, hr.assetno, hr.os, c.dist_vec, hr.charge_src, c.services order by c.post_date desc, hr.usrprinc)=1 then
    		monthagg(c.post_date) over (partition by hr.hostname, hr.assetno, hr.os, c.dist_vec, hr.charge_src, c.services 
      ...
      from 
            ...
            ,(select
                    i1.*
                    ,j.post_date
                from pireport.charges_distvec_v i1
                    ,pireport.jnls j
               where i1.jnl_id=j.jnl_id
                 and j.type='M'
