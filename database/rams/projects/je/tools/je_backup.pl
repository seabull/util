...
# $Id: je_backup.pl,v 1.2 2005/09/01 16:56:18 oracle Exp $
...
je_backup.pl	- Call regular cold backup script to do cold backup of Oracle database
...
sub parse_opts() {
	...
	if (@ARGV) {
		$DEST_DIR = shift(@ARGV);
		usage("\n\nOnly One argument is allowed.") if @ARGV;
	}
...
sub do_coldbkup($) {
	...
	#system("cp /usr/oracle/bin.qa/foo /usr/oracle/bin.qa/foo1") and die "Error executing backup script $bkup_cmd - $? - $! \n"; 
