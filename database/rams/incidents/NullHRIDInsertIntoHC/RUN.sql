...
*/
	...
	select assetno
	  into l_tmp
	  from (
        SELECT hs.hr_id,hsc.assetno,hsc.pct,hs.pct hpct,
			       hsc.charge,hsc.amount,nvl(hs.trans_date,j.post_date) trans_date,
			       hs.service_id,hsc.account,a.flag account_flag,
			       qjournal
	     ,sysdate DD, 'Monthly JE' notes
			 FROM host_service hs,host_service_charge hsc,accounts a,
			      journals j
			WHERE hs.assetno=hsc.assetno AND hs.pri=hsc.pri
			  AND hs.service_id=hsc.service_id
	  ...
	  where hr_id is null
	    and rownum<2;
	  Util.log('Found '||' null hr_id for '||l_tmp);
      ...
      for r in (
        SELECT hs.hr_id,hsc.pct,hs.pct hpct,
	     ...
	     ,sysdate DD, 'Monthly JE' notes
	  	...
	  	Util.log('Inserting host_charged '||r.hr_id||','||r.account||','||r.service_id||','||r.amount);
         	  	INSERT INTO host_charged (
			          hr_id,pct,hpct,charge,amount, trans_date,
			          service_id,account,account_flag,journal
	              ,creation_date,created_by
               ...
               (
               r.hr_id,r.pct,r.hpct,
			       r.charge,r.amount,r.trans_date,
			       r.service_id,r.account,r.account_flag,
			       r.qjournal, r.dd,r.notes
               );
	  	Util.log('Inserted host_charged '||r.hr_id||','||r.account||','||r.service_id||','||r.amount);
        END LOOP;                   
