...
-- $Header: c:\\Repository/sql/oracle/debug_pkg_ylj/debug_body.sql,v 1.2 2005/03/16 22:44:16 yangl Exp $
-- $Author: yangl $
--
/* 
  Note that this package body was developed by Longjiang Yang 
...
begin
  ...
  for c in ( select * 
             ...
             where userid = user )
  loop
    -- Check the debug level first
    if p_dlevel <= c.dlevel then
...
begin
  ...
  insert into debugtab(
    userid, dlevel, modules, filename, show_date,
    date_format, name_length, session_id )
  values (
    p_user, p_dlevel, p_modules, p_file, p_show_date,
    p_date_format, p_name_len, p_show_sesid )
  returning
    userid, dlevel, modules, filename, show_date,
    date_format, name_length, session_id
  into
    debugtab_rec.userid, debugtab_rec.dlevel, debugtab_rec.modules,
    debugtab_rec.filename, debugtab_rec.show_date,
    debugtab_rec.date_format, debugtab_rec.name_length,
    debugtab_rec.session_id;
  ...
  l_message := l_message || '         DLEVEL: ' ||
               debugtab_rec.dlevel || chr(10);
...
begin
  ...
  loop
    ...
    dbms_output.put_line( 'USER:                 ' ||
                          c.userid );
    dbms_output.put_line( 'DEBUGLEVEL:                 ' ||
                          c.dlevel );
