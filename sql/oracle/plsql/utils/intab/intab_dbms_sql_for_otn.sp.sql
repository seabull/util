-- $Header: c:\\Repository/sql/oracle/plsql/utils/intab/intab_dbms_sql_for_otn.sp.sql,v 1.1 2006/01/19 21:12:17 yangl Exp $
--

CREATE OR REPLACE PROCEDURE intab (
   schema_in         IN   VARCHAR2
  ,table_in          IN   VARCHAR2
  ,from_append_in    IN   VARCHAR2 DEFAULT NULL
...
/*
   ...
   Parameters:
      ...
      schema_in - name of the schema owning the table or view. This value will be
                 upper-cased before using it in the query.

      table_in - name of the table or view. This value will be upper-cased before
                using it in the query.

      from_append_in - Any text that can be legally appended to a SELECT
                      statement after the FROM clause.
...
IS
   ...
   --
   g_upper_table_name      all_objects.object_name%TYPE
                                                       := UPPER (table_in);
   g_upper_schema_name     all_objects.owner%TYPE     := UPPER (schema_in);
   ...
   */
   ...
   BEGIN
      ...
      BULK COLLECT INTO g_columns
       ...
       WHERE owner = g_upper_schema_name
         AND table_name = g_upper_table_name
   ...
   EXCEPTION
      ...
      THEN
         ...
         DBMS_OUTPUT.put_line
                        (   '   No columns found for object specified as '
                         || g_upper_schema_name
                         || '.'
                         || g_upper_table_name
                        );
   ...
   IS
      ...
      BEGIN
         ...
         DBMS_OUTPUT.put_line (centered_string (   'Contents of '
                                                || g_upper_table_name
                                               ,g_row_line_length
                                               )
                              );
   ...
   BEGIN
      ...
      LOOP
         /*
		   ...
		   Note: DBMS_SQL also supports an array processing interface, 
		         in which you fetch multiple rows and then use 
				 overloadings of DBMS_SQL.COLUMN_VALUE to retrieve data
				 into arrays, rather than scalar values. That approach
				 will improve the performance of this step.
         */
         ...
         LOOP
            ...
            DBMS_SQL.COLUMN_VALUE
			   ...
			   Scalar overloadings here could be replaced with 
			   collection overloadings.
            */
            IF is_string (l_index)
            THEN
