CREATE OR REPLACE PACKAGE BODY RUN IS
  ...
  BEGIN
    DEBUG.LOG(16,'Entering, qpct=%s,qname=%s,qsubname=%s,qdist=%s'
                       ,nvl(qpct,'NULL')
                       ,qname
                       ,qsubname
                       ,qdist
  ...
  BEGIN
    ...
    INSERT INTO tmcd_recorded (
        ...
        account, exp_org
    ) SELECT param.charge_last,l.princ,cs.kind,
  	         l.batch, l.trcode, l.emp_num, l.lname, l.fname, l.hours, l.hours_ot,
  	         l.hed, l.paydate,l.home_org, l.kind,
             a.id, l.exp_org
        FROM tmcd_princ l, accounting.hris_org ho,
             charge_sources cs,param,accounts a
       WHERE l.home_org=ho.org(+) AND ho.org_e=cs.org AND cs.attr LIKE '%p%'
         AND (l.project,l.task,l.award)
          IN ((a.project,a.task,a.award))
    ...
    DEBUG.LOG(16,'Exit, param biweekly_pay advanced');
  END;
	...
	SELECT min(l.period),p.charge_last,p.charge_first
  	...
  	EXCEPTION
  		WHEN NO_DATA_FOUND THEN
  		  null;
  		WHEN OTHERS THEN
        DEBUG.LOG(16,'Labor data period %s outside current period %s to %s',mperiod,fperiod,lperiod);
  		  raise_application_error(
  	...
  	INSERT INTO labor_recorded (
        ...
        pct_orig, pct_norm
  	           ...
  	           l.period, l.name, l.appointment, l.home_org,
               l.objcode, l.exporg, l.exptype, a.id,
               l.pct, l.pct
          FROM labor_princ l,accounting.hris_org ho,
               charge_sources cs,param,accounts a
         WHERE l.home_org=ho.desc1(+) AND ho.org_e=cs.org AND cs.attr LIKE '%P%'
           AND (l.project,l.task,l.award)
            IN ((a.project,a.task,a.award))
  	 ...
  	 */
    DEBUG.LOG(16,'Normalize percentage to 100');
  	FOR nr IN n LOOP
  		apct := 100;
        ...
        DEBUG.LOG(16,'%s : %s =>%s',nr.name,r.pct_orig,npct);
  		END LOOP;
  	END LOOP;
  	...
  	EXCEPTION
  	  WHEN NO_DATA_FOUND THEN
  	     null;
  	  WHEN OTHERS THEN
        DEBUG.LOG(16,'Labor did not normalize to 100');
	  	  raise_application_error(
      ...
      */
      ...
      DEBUG.LOG(16,'Advanced labor_dist to %s', asof);
		END;
    DEBUG.LOG(16,'Exit');
  END;
  ...
  PROCEDURE peopleDistNames(xprinc princ_t) IS
     ...
     */
     ...
     UPDATE tmcd_recorded tr SET
      ...
      IF (SQL%ROWCOUNT > 0) THEN
        DEBUG.LOG(16,'Mapped %s new timecard names', SQL%ROWCOUNT);
 ...
 */   
      ...
      --END IF;
	  	...
	  	SELECT /*+ MERGE_AJ*/
	  	  princ,round(sum(hours)/1.6275,2),charge_src,account
	  	FROM tmcd_recorded
	  	WHERE princ IS NOT NULL
	  	 AND period_last=(SELECT charge_last FROM param)
       ...
       DEBUG.LOG(16,'Inserted %s rows into people_dist', SQL%ROWCOUNT);
       ...
       --we now get ICES data via labor outload or 3 month interval updates via spreadsheet
	  	 ...
	  	 */
      DEBUG.LOG(16,'Adjust charge source to a composite if multiple records exit for a princ'); 
	  	FOR pr IN p LOOP
		    DECLARE
          ...
          DEBUG.LOG(16,'Composite %s is %s',pr.princ,src);
		    EXCEPTION
		    	WHEN OTHERS THEN
            DEBUG.LOG(16,'Composite charge source %s must be manually added to charge_source',pr.princ);
				  	raise_application_error(
				  	  -20198,
    ...
    DEBUG.LOG(16,'Entering');
		...
		EXCEPTION
			WHEN NO_DATA_FOUND THEN
			  null;
			WHEN OTHERS THEN
        DEBUG.LOG(16,'Charge sources not unique in peopleDistNames()');
				raise_application_error(
    ...
    DEBUG.LOG(16,'Loop');
	  FOR pr IN (
       ...
       WHERE princ NOT IN (SELECT subname FROM dist_names_x)
        ...
        DEBUG.LOG(16,'Processing %s %s',pr.princ,pr.wpct);
		  	...
		  	IF (pr.wpct != 100) THEN
		  	  tpct := pr.wpct;
		  		...
		  		FOR r IN c(pr.princ) LOOP
		  			...
		  			END IF;
            DEBUG.LOG(16,'Normalize100 %s => %s',r.pct,npct);
 ...
 a.src_account IS NOT NULL)
		      ...
		      IF (SQL%ROWCOUNT > 0) THEN
            DEBUG.LOG(16,'Substituted %s per-user accounts',SQL%ROWCOUNT);
		      ...
		      IF (SQL%ROWCOUNT > 0) THEN
            DEBUG.LOG(16,'Substituted %s any user accounts',SQL%ROWCOUNT);
	...
	--Util.log(to_char(sysdate,'MI:SS')||' before subs removals');
		      ...
		      IF (SQL%ROWCOUNT > 0) THEN
            DEBUG.LOG(16,'Removed %s per-user accounts', SQL%ROWCOUNT);
		      ...
		      UPDATE dist SET tpct=0 WHERE dist=0
		    	...
		    	IF (SQL%ROWCOUNT > 0) THEN
            DEBUG.LOG(16,'Removed %s any user accounts',SQL%ROWCOUNT);
	...
	--Util.log(to_char(sysdate,'MI:SS')||' after subs removals');
	       ...
	       */
        DEBUG.LOG(16,'Renormalize the new distribution'); 
	  		...
	  		IF (tpct != 0) THEN
		  		FOR dr IN d LOOP
		  			IF (dr.tpct != 0) THEN
			  			...
			  			END IF;
              DEBUG.LOG(16,'Normalize %s => %s',dr.tpct,npct);
          ...
          DEBUG.LOG(16,'%s (%s) has distribution %s',nvl(pr.princ,'NULL'),pr.charge_src,did);
    ...
    DEBUG.LOG(16,'End Loop');
      ...
      DEBUG.LOG(16,'Purged %s effort mappings withno current charging for ppe=%s', SQL%ROWCOUNT,ppe);
    ...
    DEBUG.LOG(16,'Exit, advanced param.dist_named to next month');
  END;
  ...
  BEGIN
    ...
    DEBUG.LOG(16,'Loop');
  	FOR r IN (
        ...
        DEBUG.LOG(16,'Unchanged %s',r.subname); 
		...
		IF (SQL%ROWCOUNT = 0) THEN
		    ...
		    Util.log ('Insert '||r.subname||' to '||r.ndist||','||
		       r.nsrc||',' ||r.npct||'%');
        DEBUG.LOG(16,'Insert subname=%s to ndist=%s,nsrc=%s,npct=%s',r.subname,r.ndist,r.nsrc,r.npct);
		ELSE
        DEBUG.LOG(16,'Update dist_names subname=%s to dist=%s,src=%s,pct=%s',r.subname,r.ndist,r.nsrc,r.npct);
     ...
     *  or summed to zero.
     */
    DEBUG.LOG(16,'Loop, remove effort mapping for people no longer defined or summed to zero'); 
    FOR r IN (
      ...
      DEBUG.LOG(16,'Purge person %s:%s',r.subname,SQL%ROWCOUNT);
    END LOOP;
    DEBUG.LOG(16,'End Loop');
     ...
     */
    DEBUG.LOG(16,'Loop, remove dist for people zeroed in effort who went to residual status from the above purge.');
    FOR r IN (
      SELECT princ FROM people_dist GROUP BY princ
       HAVING sum(pct) =0
      ...
      DEBUG.LOG(16,'Zero person %s:%s',r.princ,SQL%ROWCOUNT);
  ...
  DEBUG.LOG(16,'Exit, advance processing date to next month');
 ...
 already '||already||', left '||dr.left||', asof '||asof||', until '||until||', insts '||installments);
  ...
  END;   
    ...
    DEBUG.LOG(16,'Journaled accumulated depreciation creadit');
		--Util.log('Journaled accumulated depreciation credit');
    ...
    DEBUG.LOG(16,'Journaled depreciation expense');  
		--Util.log('Journaled depreciation expense');
		/*
		 *  These two entries log a net 0 expense to breakdown
		 *  depreciation by category for future expense analysis
     ...
     SELECT qjournal,d.objcode,qpost_date,
      ...
      group by d.objcode,d.description;
		--Util.log('Logged depreciation credit');
    DEBUG.LOG(16,'Logged depreciation credit');
		/* log depreciation expenses by category 
     *
     ...
     */
      ...
      and period_end is null
		...
		--Util.log('Logged depreciation expenses by category');
    DEBUG.LOG(16,'Logged depreciation expenses by category');
    DEBUG.LOG(16,'Exit');
  END;
  ...
  PROCEDURE journalRecord(recapture BOOLEAN, qjournal journal_t) IS
    ...
    DEBUG.LOG(16,'Purged %s rows from dist_names_X', SQL%ROWCOUNT);
    --Util.log('Purged '||SQL%ROWCOUNT||' dist_names_x records');
	  	DELETE from host_charged
	  	 WHERE journal=qjournal;
      DEBUG.LOG(16,'Purged %s rows from host_charged',SQL%ROWCOUNT);
      ...
      DEBUG.LOG(16,'Purged %s rows from who_charged', SQL%ROWCOUNT);
      DEBUG.LOG(16,'Exit');
	  END;

  	PROCEDURE capture (qjournal journal_t) IS
  	BEGIN
      DEBUG.LOG(16,'Entering, journal=%s',qjournal);
      ...
      /*
	    ...
	    INSERT INTO host_recorded (
         ...
         WHERE h.assetno IN (select assetno FROM host_service)
           AND h.hr_id IS NULL;
      DEBUG.LOG(16,'Sync host_recorded table, inserted %s new rows in host_recorded',SQL%ROWCOUNT);     
		   ...
		   *  all its priorities.
		   */
      DEBUG.LOG(16,'Loop, host_service ID updates');
      ...
      FOR r IN (
        ...
        DEBUG.LOG(16,'ID=%s from OID=%s for assetno=%s.pri=%s,count=%s',r.nid,r.oid,r.assetno,r.pri,SQL%ROWCOUNT);
      END LOOP;
      ...
      DEBUG.LOG(16,'End Loop');
	     ...
	     */
          ...
          ,creation_date,created_by
			       ...
			       hs.service_id,hsc.account,a.flag account_flag,
			       qjournal
             ,sysdate, 'Monthly JE'
      ...
      DEBUG.LOG(16,'Inserted %s host_charged records',SQL%ROWCOUNT);
	  	...
	  	DEBUG.LOG(16,'Logged %s non-monthly host services');
      ...
      */
	    ...
	    INSERT INTO who_recorded (
         ...
         WHERE w.princ IN (select princ FROM who_service)
           AND w.wr_id IS NULL;
	  	DEBUG.LOG(16,'Inserted %s new who_recorded records', SQL%ROWCOUNT);
      ...
      DEBUG.LOG(16,'Loop, who_service ID updates');
      FOR r IN (
        ...
        DEBUG.LOG(16,'ID=%s from oid=%s for princ=%s,count=%s',r.nid,r.oid.r.princ,SQL%ROWCOUNT);
      ...
      DEBUG.LOG(16,'End Loop');
	    ...
	    INSERT INTO who_charged (
          ...
          ,creation_date,created_by
			       ...
			       ws.service_id,sc.account,a.flag account_flag,qjournal
             ,sysdate,'Monthly JE'
      ...
      DEBUG.LOG(16,'Inserted %s who_charged records',SQL%ROWCOUNT);
	  	--Util.log('Inserted '||SQL%ROWCOUNT||' who_charged records');

	  	UPDATE who_service SET journal=qjournal
	  	 WHERE service_id IN (SELECT id FROM services WHERE monthly IS NULL);
      DEBUG.LOG(16,'Logged %s non-monthly user services',SQL%ROWCOUNT);
	...
	UPDATE param SET journal_entry=labor_dist;
	  	COMMIT;
      DEBUG.LOG(16,'Exit, advanced param.journal_entry to next month.');
  	END;
  ...
  BEGIN
    ...
    DEBUG.LOG(16,'Purged %s journal records',SQL%ROWCOUNT);
    --Util.log('Purged '||SQL%ROWCOUNT||' obsolete journal records');
    DELETE FROM expense WHERE journal =	qjournal;
    DEBUG.LOG(16,'Purged %s records from expense',SQL%ROWCOUNT);
	...
	ELSE
	     ...
	     *  being rejected).
	     */
      DEBUG.LOG(16,'Update account flags in charged tables');
	    ...
	    DEBUG.LOG(16,'Adjusted %s account flags in host_charged',SQL%ROWCOUNT);
      --Util.log('Adjusted '||SQL%ROWCOUNT||' account flags in host_charged');
	    ...
	    DEBUG.LOG(16,'Adjusted %s account flags in who_charged',SQL%ROWCOUNT);
      ...
      ,creation_date,created_by
  	)select qjournal,88200,c.account,sum(c.amount),qpost_date,
		 ...
		 WHERE c.journal=qjournal AND c.account_flag IS NULL
		   AND hr.id=c.hr_id
		 group by c.account;
    DEBUG.LOG(16,'Journaled %s machine records', SQL%ROWCOUNT);
    --Util.log('Journaled '||SQL%ROWCOUNT||' machine records');
      ...
      ,creation_date,created_by
		 ...
		 where c.journal=qjournal and c.account_flag is null
		   AND wr.id=c.wr_id
		 group by c.account;
    DEBUG.LOG(16,'Journaled %s user records',SQL%ROWCOUNT);
    --Util.log('Journaled '||SQL%ROWCOUNT||' user records');
	   ...
	   */
		...
		) SELECT qjournal,'68200',qpost_date,
		       sum(j.amount), 'CO',
    ...
    DEBUG.LOG(16,'Logged GL expense');    
		...
		DEBUG.LOG(16,'Logged GM expense');
    --Util.log('Logged GM expense');

		/* log user revenue by category */
		INSERT INTO expense(journal,objcode,post_date,amount,category,description ,catbyprinc)
		 SELECT qjournal,'68200',qpost_date, 0-sum(wc.amount),
		  ...
		  GROUP BY c.name,c.description ;
    DEBUG.LOG(16,'Logged user revenue by category');
		--Util.log('Logged user revenue by category');
		/* log host revenue by category */
		INSERT INTO expense(journal,objcode,post_date,amount,category,description ,catbyprinc)
		 SELECT qjournal,'68200',qpost_date, 0-sum(hc.amount),
		  ...
		  GROUP BY c.name,c.description;
		DEBUG.LOG(16,'Logged host revenue by category');
  ...
  BEGIN
     ...
     * view is used to retrieve a more reliable id
  	 */    
    journal := get_open_je;
    DEBUG.LOG(16,'get journal=%s, set journal in process flag',journal);
    UPDATE JOURNALS 
   ...
   */
	jnum := get_open_je;
  DEBUG.LOG(16,'Get journal=%s to accept',jnum);
  SELECT COUNT(*) INTO n FROM journal 
  WHERE journal = jnum;
	IF (n = 0) THEN
        DEBUG.LOG(16,'Exit, ERROR:no journal entry to accept');
     	  raise_application_error(
	...
	END IF;
	  ...
	  DEBUG.LOG(16,'Removed %s non-monthly host_service records',SQL%ROWCOUNT);
    --Util.log('Removed '||SQL%ROWCOUNT||' non-monthly host_service records');
  	DELETE FROM who_service
  	 WHERE journal IN (SELECT journal FROM param);
	  DEBUG.LOG(16,'Removed %s who_service records',SQL%ROWCOUNT);
    ...
    */
    ...
    WHERE  id = jnum;
    DEBUG.LOG(16,'unset the journal in process flag');
	  /*
   ...
   --Util.log('Purged '||SQL%ROWCOUNT||' zero ICES records');
    ...
    UPDATE capequip SET warranty_expire=warranty_expire
     WHERE assetnum IN (
	  ...
	  DEBUG.LOG(16,'Updated %s now non-warranted assets', SQL%ROWCOUNT);
     ...
     */
     ...
     /* 
     update the GL_REPORT table to insert service codes
     */
     /*
     hostdb.update_gl_report_services.SWITCH(jnum,'M',retcode,errdesc);
     */
     DEBUG.LOG(16,'Exit');
	END;
  ...
  BEGIN
    ...
    update hostdb.accounts 
         ...
         and flag = 'l'; 
    DEBUG.LOG(16,'unlimbo-ed %s GM accounts',SQL%ROWCOUNT);
    ...
    update hostdb.accounts 
         ...
         and flag = 'l'; 
    DEBUG.LOG(16,'unlimbo-ed %s GL accounts',SQL%ROWCOUNT);
    DEBUG.LOG(16,'Exit');
   END unLimbo; 
   ...
   DEBUG.LOG(16,'Exit,journal=%s',jnum);
   RETURN jnum; 
END get_open_je;

  PROCEDURE update_services(jnum journal_t) IS
    errdesc        varchar2(50);
    retcode        varchar2(50);
  BEGIN
       DEBUG.LOG(16,'Entering');
