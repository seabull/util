...
--IF OBJECT_ID('dbo.Util_IndexList', 'P') IS NOT NULL DROP PROCEDURE dbo.Util_IndexList
--GO
...
/**
    *=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=
    ...
    If you want data types and other column information and one row per index instead of one row per member column,
      then use Util_IndexColumnList instead.
    ...
    Optional Input Parameters
    	...
    	@TableName sysname=''       Filters to a single table.  Can use LIKE wildcards.  
                                    All tables if blank.  Accepts LIKE Wildcards.
    	@Delimiter VarChar(1)=','   Delimiter for the horizontal delimited seek and include column listings. 
                                    For accomdating csv export.
    
    Usage
    	EXECUTE Util_IndexList 'dbo', 'FORTVault'
    
    *=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=
**/

CREATE PROCEDURE dbo.Util_IndexList
	@SchemaName sysname='',
	@TableName sysname='',
	@Delimiter VarChar(1)=','
AS
