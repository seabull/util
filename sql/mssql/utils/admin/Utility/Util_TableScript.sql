 ...
 BEGIN
    ...
    INSERT INTO @ShowFields
     ...
     INNER JOIN sys.columns sc    
            --ON  c.TABLE_NAME = OBJECT_NAME(sc.object_id) AND c.COLUMN_NAME = sc.Name
            ON  OBJECT_ID(c.TABLE_SCHEMA + '.' + c.TABLE_NAME) = sc.object_id 
     LEFT JOIN sys.identity_columns ic 
            ON c.TABLE_NAME = OBJECT_NAME(ic.object_id) AND c.COLUMN_NAME = ic.Name
...
--
     ...
     VALUES('CREATE TABLE ' + CASE WHEN @NewTableName IS NOT NULL 
                                    THEN @NewTableName 
     ...
     INSERT INTO @Definition(FieldValue)
         SELECT
                 CHAR(10) + FieldName + ' ' +
                            CASE WHEN DomainName IS NOT NULL AND @UseSystemDataTypes = 0 
                                THEN DomainName + CASE WHEN IsNullable = 1 THEN ' NULL ' ELSE ' NOT NULL ' END
                                ELSE UPPER(DataType) +
                                   CASE WHEN IsCharColumn = 1 THEN '(' + CAST(MaxLength AS VARCHAR(10)) + ')' ELSE '' END +
                                   CASE WHEN IdentityColumn = 1 
                                        THEN ' IDENTITY(' + CAST(IdentitySeed AS VARCHAR(5))+ ',' 
                                                + CAST(IdentityIncrement AS VARCHAR(5)) + ')' ELSE '' END
                                  + CASE WHEN IsNullable = 1 THEN ' NULL ' ELSE ' NOT NULL ' END 
                                  + CASE WHEN ColumnDefaultName IS NOT NULL AND @IncludeConstraints = 1 
