...
# $Archive:   //cs01pvcs/pvcs/cm/Database/archives/Applications/DBINST/SQL65/T38size.pv_  $
# $Author: A645276 $
...
#*
#*
#* Purpose:  T38size.pl calculates the size of databases checks the log file for errors.  If it finds an error it exit with status code 1.
#* Program must be executed from within the directory it is located.
#*
#* SYNOPSIS
#*
#*	t38size.pl -h | -S server name
#*
#*	Command line:
#*
...
# Open log file 
unless (open(AppendLog,">>$ghistoryFile")) {
	print ("** ERROR ** [main]: Cannot open file $ghistoryFile: $!");
	exit 1;
}
...
sub append {
	...
	($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst) = localtime(time);
	printf AppendLog ("%02d/%02d/%02d %02d:%02d \n %s\n", $mon+1, $mday, $year, $hour, $min, $appendmsg);
} # END sub append
...
my ($dbSize, $startDbSize, $finishDbSize);

$dbSize = `$gisql -Q "exec sp_T38SCANDB "`;
&notifyme ("Start DB Size \n");
&notifyme ("---------------- \n");
&notifyme ("$dbSize \n");
&notifyme ("Finish DBSize \n");
}
