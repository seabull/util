...
#*
#* SYNOPSIS
#*	t38trace -h -a 10 -l logfileDirSuffix -n nTraces -S server -s debugSQLFile -x{s|x} -f filters trcParms
#*	
#*	Where:
#*	
...
#*  -d trace out put destination folder, defalut override. 
...
#*
#*	Maintenance Log,  
#*		Matthew Ruppert 04/19/2007   Change to add new ARG for optional destination for trace.
#*
#***
...
my ( $gCurrentDir, $gScriptName, $gScriptPath, $gHostName, $gNWarnings, $gClusterPath) = "";
...
use constant SQL8MAXEVENT	=> 118;	# SQL 8 highest event id
...
use constant SQL8MAXCOL	=> 44;	# SQL 8 highest column id
...
my %gTrcColumns		= 		# SQL Trace data columns to be used for the trace.
	...
	LinkedServerName	=> {SQL8FltrHdlr => \&fltHdlrSql8str, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 45, SQL7ColmId => 0},
	ProviderName		=> {SQL8FltrHdlr => \&fltHdlrSql8str, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 46, SQL7ColmId => 0},
	MethodName			=> {SQL8FltrHdlr => \&fltHdlrSql8str, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 47, SQL7ColmId => 0},
	RowCounts			=> {SQL8FltrHdlr => \&fltHdlrSql8num, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 48, SQL7ColmId => 0},
	RequestID			=> {SQL8FltrHdlr => \&fltHdlrSql8num, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 49, SQL7ColmId => 0},
	XactSequence		=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 50, SQL7ColmId => 0},
	EventSequence		=> {SQL8FltrHdlr => \&fltHdlrSql8num, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 51, SQL7ColmId => 0},
	BigintData1			=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 52, SQL7ColmId => 0},
	BigintData2			=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 53, SQL7ColmId => 0},
	GUID				=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 54, SQL7ColmId => 0},
	IntegerData2		=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 55, SQL7ColmId => 0},
	ObjectID2			=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 56, SQL7ColmId => 0},
	Type				=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 57, SQL7ColmId => 0},
	OwnerID				=> {SQL8FltrHdlr => \&fltHdlrSql8num, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 58, SQL7ColmId => 0},
	ParentName			=> {SQL8FltrHdlr => \&fltHdlrSql8str, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 59, SQL7ColmId => 0},
	IsSystem			=> {SQL8FltrHdlr => \&fltHdlrSql8num, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 60, SQL7ColmId => 0},
	Offset				=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 61, SQL7ColmId => 0},
	SourceDatabaseID	=> {SQL8FltrHdlr => \&fltHdlrSql8num, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 62, SQL7ColmId => 0},
	SqlHandle			=> {SQL8FltrHdlr => 0, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 63, SQL7ColmId => 0},
	SessionLoginName	=> {SQL8FltrHdlr => \&fltHdlrSql8str, SQL7FltrHdlr => 0, SQL7spName => 0, SQL8ColmId => 64, SQL7ColmId => 0},
	);
...
sub startSQL8trace () {
	...
	my $maxfilesize		= $gSQL8MAXTRCSIZE;	# Maximum trace file size before rollover
SUB:
{
	&notifyWSub("Started.");
...
#mldr replaced with line below	getopts('ha:l:n:S:s:x:f:');
	getopts('ha:l:n:S:s:x:f:d:');
	...
	$gClusterPath	= '';			# SQL Server Cluster name to be appended to share drive paths
	$gDebugSQL	= '';
	$gNWarnings	= 0;
	$gSQL8MAXTRCSIZE = SQL8MAXTRCSIZE;
	...
	SUB: {
		...
		#  Insert new code to check for Clustering
		...
		# Find ServerProperties.
		$sql	= << '		EOT';
			if (@@version like 'Microsoft SQL Server  7.00%')
				select 0 AS IsClustered
			else
				exec ('select ServerProperty(''IsClustered'') AS IsClustered')
		EOT

		debugPrintSQL($sql);
		unless(&execSQL2Arr($adocmd, $sql, \@rs)) { $status = 0; last SUB; }

		if ($rs[0]{IsClustered} == 1) {
			$gClusterPath = ".$gNetName";
#			&errme("Clustered FOUND! $gScriptPath"."T38LOG\\$gScriptName");
		}
		...
		if ($#ARGV != 0 ) {
			...
			my $t38trcPath	= "\\\\$gNetName\\t38trc$gClusterPath";
# start mldr
			if ($Getopt::Std::opt_d) { 
				$t38trcPath = $t38trcPath . "\\$Getopt::Std::opt_d";
				}
# end mldr
#		
			&notifyWSub("Trace File Path: $t38trcPath");

			# Create Directory if they not there for ArchSight or others
			unless (-d $t38trcPath) {
				&notifyWSub("Trace directory $t38trcPath is missing.");
				&notifyWSub("Creating directory $t38trcPath");
				&mkdir($t38trcPath);
			}
		...
		if (defined( $gConfigValues{'trcFile:MAXSQL8FILESIZE'}) && $gConfigValues{'trcFile:MAXSQL8FILESIZE'} > 0) {
			$gSQL8MAXTRCSIZE = $gConfigValues{'trcFile:MAXSQL8FILESIZE'};
		}
		...
		if ( ($gSQLVersion == 0) || !($gSQLVersion =~ s/^(\d+.\d+)\..+/$1/) ) {
		...
		if ($gSQLVersion !~ /^7\.00|8\.00|9\.00|10\.0$/) {
		...
		if ($gRunOpt eq 's') {
			...
			my $akey;
			foreach $akey (@gDoEvents) {
				if (!defined $gTrcEvents{$akey} || 
					($gSQLVersion eq '7.00' && $gTrcEvents{$akey} > SQL7MAXEVENT) ||
					($gSQLVersion eq '8.00' && $gTrcEvents{$akey} > SQL8MAXEVENT)) {
			...
			foreach $akey (keys %gDoFilters) {
				if (!defined $gTrcColumns{$akey} || 
					...
					($gSQLVersion eq '9.00' && !$gTrcColumns{$akey}{SQL8FltrHdlr}) ||
					($gSQLVersion eq '10.0' && !$gTrcColumns{$akey}{SQL8FltrHdlr})
				) {
			...
			foreach $akey (@gDoColumns) {
				if (!defined $gTrcColumns{$akey} || 
					...
					($gSQLVersion eq '8.00' && ($gTrcColumns{$akey}{SQL8ColmId} > SQL8MAXCOL)) ||
					($gSQLVersion eq '9.00' && !$gTrcColumns{$akey}{SQL8ColmId}) ||
					($gSQLVersion eq '10.0' && !$gTrcColumns{$akey}{SQL8ColmId})
				) {
		...
		($gRunOpt eq 'x') ?
			...
			;
		($gRunOpt eq 's' && $gSQLVersion ne '7.00') ?
			&notifyWSub("Max Trace File size is $gSQL8MAXTRCSIZE MB.") : 0;
...
#*				filters will overwrite SQL Trace filters, created by default.
#* mldr start
#*	-d dir		Overrides the default directory for the trace output.
#* mldr start
#*
#*	trcParms	configuration file with standard sql trace parameters.
#*	
