...
#
# 
#    Rev 1.10   May 07 2007 09:45:00   tsmmxr
# Added code to check for errors after each execution of nextRecordset and MoveNext(record).
# 
#    Rev 1.9   May 02 2007 14:45:16   tsmmxr
# Added option to adoConnect subrouting to create encrypted connection.
# 
...
#		useSSL		use SSL Encryption to create connection.
#	return:
#		command object for ADO connection. Command name is "cmd1"
#		0	Failure
# ----------------------------------------------------------------------
...
sub adoConnect ($$;$$) {
	...
	my $cmd			= 0;		# ADO Command object handle
	my $sqlerr		= 0;
	my $parmSSL		= ($useSSL)? 'Encrypt=true;':'';	# Connection parameter for SSL Connection.
...
SUB:
{
	...
	$conn->{ConnectionString} = "Provider=SQLOLEDB;Server=$sqlFullName;Initial Catalog=$dbName;Trusted_Connection=Yes;${parmSSL}Application Name=$appName;Connect Timeout=30;OLE DB Services=-4;";
	...
	while($rs && !$rs->EOF) {
		...
		if ($fldCount) {
			...
			while($rs->Fields->count && ! $rs->EOF) {
				...
				$rs->MoveNext;
				if ( !&isADOok($sqlerr)) { 
					&errme("Error in SQL"); &notifyWSub(" <- Problem caller.", 2);
					&notifyMe("SQL batch:\n$sql");
					&showADOErrors($sqlerr); 
					$status = 0;
				}
