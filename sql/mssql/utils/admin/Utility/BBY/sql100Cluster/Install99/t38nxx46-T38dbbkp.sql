...
** $Archive:   //uxpvcs2/pvcs/projects/Database/archives/Applications/DBINST/SQL80/t38nxx46-T38dbbkp.svl  $
...
select "
Start of script" = convert(varchar(8), getdate(), 1) + ' ' + convert(varchar(8), getdate(), 8) + ': $Workfile:   t38nxx46-T38dbbkp.sql  $, $Revision: 1.1 $'
go
...
IF (SELECT COUNT(*) FROM msdb.dbo.syscategories WHERE name = N'Database Maintenance') < 1 
  ...
  WHERE (name = N'T38Nxx46-T38dbbkp')       
  ...
  BEGIN 
    ...
    RAISERROR (N'Unable to import job ''T38Nxx46-T38dbbkp'' since there is already a multi-server job with this name.', 16, 1) 
    GOTO QuitWithRollback  
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_name = N'T38Nxx46-T38dbbkp' 
    SELECT @JobID = NULL
  END 
...
BEGIN 
  ...
  begin
      ...
      select @serverOpt = ' -S ' + @instsrvname + ' -c '+ @scriptDir + '\t38dba.cfg' + ' -b db '
  end
  else
  ...
  begin
	...
	select @serverOpt = ' -S .\' + cast (serverproperty('InstanceName') as varchar(128)) + ' -c '+ @scriptDir + '\t38dba.cfg' + ' -b db '
  end
  else 
  begin
	select @scriptDir = '\\%computername%\t38app80'
	select @serverOpt = ' -c '+ @scriptDir + '\t38dba.cfg' + ' -b db '
  end
...
select "
End of script" = convert(varchar(8), getdate(), 1) + ' ' + convert(varchar(8), getdate(), 8) + ': $Workfile:   t38nxx46-T38dbbkp.sql  $, $Revision: 1.1 $'
go
